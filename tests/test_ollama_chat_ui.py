...
def _make_provider(models: list[ModelInfo] | None = None) -> MagicMock:
    provider = MagicMock(spec=Provider)
    if models is not None:
        provider.list_models.return_value = models
    else:
        provider.list_models.return_value = _mock_models("llama3.1")
...
class TestChatLoopModelSelection:
    def test_ollama_unavailable(self) -> None:
        provider = MagicMock(spec=Provider)
        provider.list_models.side_effect = ProviderUnavailable("Not available")
        term = FakeTerminal(keys=[" "])
        chat_loop(term, provider, "llama3.1")
        output = term.output
        assert "not available" in output

    def test_no_models_installed(self) -> None:
        provider = _make_provider(models=[])
        term = FakeTerminal(keys=[" "])
        chat_loop(term, provider, "llama3.1")
        output = term.output
    ...
    def test_single_model_auto_selected(self) -> None:
        provider = _make_provider(models=_mock_models("llama3.1"))
    ...
    def test_multiple_models_shows_selection(self) -> None:
        provider = _make_provider(models=_mock_models("deepseek", "llama3.1", "mistral"))
    ...
    def test_cancel_model_selection_returns_to_menu(self) -> None:
        provider = _make_provider(models=_mock_models("deepseek", "llama3.1"))
        term = FakeTerminal(keys=["q"])
        chat_loop(term, provider, "llama3.1")
        output = term.output
...
class TestChatLoopStreaming:
    ...
    def test_conversation_accumulates_context(self) -> None:
        ...
        second_call_messages = provider.chat_stream.call_args_list[1][1]["messages"]
    ...
    def test_esc_cancels_generation(self) -> None:
        provider = _make_provider()
        provider.chat_stream.return_value = iter(["one ", "two ", "three "])
...
class TestChatLoopCommands:
    ...
    def test_model_list_shows_installed(self) -> None:
        provider = _make_provider(models=_mock_models("llama3.1", "mistral"))
...
class TestChatLoopErrors:
    def test_model_not_found(self) -> None:
        provider = _make_provider()
        provider.chat_stream.side_effect = ProviderModelNotFoundError("not found")
        term = FakeTerminal(keys=["hello", "/quit"])
        chat_loop(term, provider, "llama3.1")
        output = term.output
    ...
    def test_connection_error_during_chat(self) -> None:
        provider = _make_provider()
        provider.chat_stream.side_effect = ProviderUnavailable("Connection refused")
        term = FakeTerminal(keys=["hello", "/quit"])
        chat_loop(term, provider, "llama3.1")
        output = term.output
    ...
    def test_invalid_response_during_chat(self) -> None:
        provider = _make_provider()
        provider.chat_stream.side_effect = ProviderResponseError("bad response")
        term = FakeTerminal(keys=["hello", "/quit"])
        chat_loop(term, provider, "llama3.1")
        output = term.output
        assert "Invalid response" in output or "bad response" in output
